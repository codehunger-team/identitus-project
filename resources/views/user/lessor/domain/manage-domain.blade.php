...
<div class="row">
	<div class="col-xs-12 col-md-8">
		...
		<input type="text" required name="domain" value="{{ $d->domain }}" class="form-control"><br />
		</div>
		...
		<div class="col-xs-12 col-md-6">
			...
			<select name="registrar_id" class="form-control">
				@foreach($registrars as $registrar) 
					<option value="{{$registrar->id}}" @if($d->registrar == $registrar->id) selected @endif>{{$registrar->registrar}}</option>
				@endforeach
			</select>
			<br />
		</div>
		...
		<div class="col-xs-12 col-md-12">
			...
			<textarea name="description" id="editor" class="form-control textarea" rows="8">{{ $d->description }}</textarea>
		<br />
		</div>
