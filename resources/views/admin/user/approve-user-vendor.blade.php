...
@section('section_title')
    ...
    <br><br>
    <a href="/admin/users" class="btn btn-default btn-xs">Back to User's List</a>
@endsection
...
<form method="POST" enctype="multipart/form-data" action="{{url('admin/vendor-approval')}}">
    ...
    <div class="col-xs-12 col-md-6">
        <label>Email</label>
        <input type="email" class="form-control periodPayment" readonly  placeholder="email" value="{{$user->email ?? ''}}">
    </div>
    <div class="col-xs-12 col-md-6">
        <label>Phone</label>
    ...
    <div class="col-xs-12 col-md-6">
        <label>Company</label>
        <input type="text" class="form-control period" readonly value="{{$user->company ?? ''}}" placeholder="company">
    </div>
    <div class="col-xs-12 col-md-6">
        <label>Country</label>
    ...
    <div class="col-xs-12 col-md-6">
        ...
        <input type="text" class="form-control" readonly value="{{$user->bankDetail->bank_name ?? ''}}">
    </div>
    <div class="col-xs-12 col-md-6">
        <label>Bank IFSC Code</label>
        <input type="text" class="form-control" readonly value="{{$user->bankDetail->bank_ifsc_code ?? ''}}">
    </div>
    <div class="col-xs-12 col-md-6">
        <label>Account Holder Name</label>
        <input type="text" class="form-control"  readonly  value="{{$user->bankDetail->account_holder_name ?? ''}}">
    </div>
    <div class="col-xs-12 col-md-6">
        <label>Account Number</label>
        <input type="text" class="form-control" readonly value="{{$user->bankDetail->account_number ?? ''}}">
    </div>
    <div class="col-xs-12 col-md-6">
        <label>Stripe ID</label>
        <input type="text" class="form-control"  readonly value="{{$user->bankDetail->stripe_id ?? ''}}">
        <br>
    </div> --}}
    @if($user->is_vendor != 'yes')
