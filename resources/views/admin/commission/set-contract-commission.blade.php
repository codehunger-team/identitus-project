...
@section('section_body')
    ...
    <form method="post" class="commission-form" action="{{route('store.contract.commission')}}">
        ...
        <div class="after-add-more tr_clone">
            <div class="col-sm-4">
                <label>From($)</label><br />
                <input type="number" name="from_domain[]" value="{{$commission->from ?? ''}}"
                    class="form-control copy-row" required placeholder="from">
            </div>
            <div class="col-sm-4">
                <label>To($)</label><br />
                <input type="number" name="to_domain[]" value="{{$commission->to ?? ''}}"
                    class="form-control copy-row" required placeholder="to">
            </div>
            <div class="col-sm-4">
                <label>Percentage(%)</label><br />
                <div class="input-group control-group">
                    <input type="number" name="commission[]" value="{{$commission->price ?? ''}}"
                        class="form-control copy-row" required placeholder="Percentage (%)">
                    <div class="input-group-btn">
                        <button class="btn btn-success add-more" type="button"><i
                                class="glyphicon glyphicon-plus"></i> Add</button>
                        <button class="btn btn-danger remove" id="{{$commission->id}} " type="button"><i
    ...
    <form method="post" action="{{route('store.contract.commission')}}">
        ...
        <div class="row after-add-more tr_clone">
            ...
            <div class="col-sm-4">
                <label>Percentage (%)</label><br />
                <div class="input-group control-group">
                    <input type="number" name="commission[]" class="form-control copy-row" required
